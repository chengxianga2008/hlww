	...
	public function get_indexed_entries($no_filter = false)
	{
		...
		foreach($entries as $entry_id => &$entry)
		{
			...
			foreach($entry as $entry_key => $entry_val ) {
				$entry[$entry_key] = apply_filters( 'laborator_data_opt_entry_key_value', $entry_val, $entry_key );
			}
		}
	...
	public function render_table_image($entry_id, $entry, $field_id)
	{
		if(is_array($entry)):
			...
			<div class="image_render">
				<a href="<?php echo home_url( apply_filters( 'laborator_data_opt_image_src', $original ) ); ?>" target="_blank">
					<img src="<?php echo home_url( apply_filters( 'laborator_data_opt_image_src', $thumbnail ) ); ?>" alt="<?php echo "image-{$entry_id}"; ?>"<?php echo $thumbnail_width ? " width=\"{$thumbnail_width}\"" : ''; echo $thumbnail_height ? " height=\"{$thumbnail_height}\"" : ''; ?> />
				</a>
			</div>
			<?php
