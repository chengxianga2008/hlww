					...
					foreach($supported_payments as $payment_method):
						...
						<li>
							<a<?php echo $link ? " href=\"{$link}\"" : ''; echo $link && $blank_page ? ' target="_blank"' : ''; ?> class="payment-slide<?php echo isset($p_image_2['original']) ? ' hover' : ''; ?>">
								<img src="<?php echo home_url($p_image_1['original']); ?>" class="normal-img" alt="<?php echo sanitize_title( $name ); ?>" />

								<?php if(isset($p_image_2['th'])): ?>
								<img src="<?php echo home_url($p_image_2['original']); ?>" class="hover-img" alt="<?php echo sanitize_title( $name ); ?>" />
								<?php endif; ?>
							</a>
						</li>
						<?php
