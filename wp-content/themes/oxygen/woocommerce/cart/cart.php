			...
			<?php do_action( 'woocommerce_before_cart_table' ); ?>
				...
				<div class="row spread cart-header-row cart-item-row">
						...
						if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
							...
							<div class="row cart-item-row <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
								...
								<div class="col col-sm-5 col-thumb-name">
									<?php
										echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf( '<a href="%s" class="remove" title="%s"></a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), __( 'Remove this item', 'oxygen' ) ), $cart_item_key );
									?>
									...
									<?php
										...
										echo WC()->cart->get_item_data( $cart_item );
					       				...
					       				if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) )
					       					echo '<p class="backorder_notification">' . __( 'Available on backorder', 'oxygen' ) . '</p>';
									?>
									</div>
					...
					<div class="cart-buttons-hidden">
						<input type="submit" class="button" name="update_cart" value="<?php _e( 'Update Cart', 'oxygen' ); ?>" />
						<input type="submit" class="checkout-button button alt wc-forward" name="proceed" value="<?php _e( 'Proceed to Checkout', 'oxygen' ); ?>" />
